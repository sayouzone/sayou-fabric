...
class OpenDartFinanceParser:
    ...
    FINANCIAL_STATEMENTS_TYPES = {
        ...
        "CIS1": (
            ...
            "세후",
            None,
        ),  # CIS (Consolidated Income Statement) : 포괄손익계산서
        ...
        "DCIS1": (
            ...
            "기능별분류",
            "세후포괄손익",
        ),  # DCIS (Detailed Consolidated Income Statement) : 단일 포괄손익계산서
    ...
    def finance(
        ...
        indicator_code: str | IndexClassCode = IndexClassCode.PROFITABILITY,
    ) -> List[Any]:
        """
        OpenDart 정기보고서 재무정보
        ...
        if api_no == FinanceStatus.SINGLE_COMPANY_FINANCIAL_STATEMENT:
            ...
            or api_no == FinanceStatus.MULTI_COMPANY_KEY_FINANCIAL_INDICATOR
        ):
        ...
        if api_no == FinanceStatus.SINGLE_COMPANY_MAIN_ACCOUNTS:
            ...
            return [
                SingleCompanyKeyFinancialIndicatorData(**data) for data in data_list
            ]
    ...
    def balance_sheet(
        self, corp_code: str, year: str, quarter: int, financial_statement: str = "CFS"
    ):
        ...
        if self._df_bs.empty or not (self._df_bs["yearly"] == yearly).any():
            self._financial_statements(
                corp_code, year, quarter, financial_statement=financial_statement
            )

        df_bs = self._df_bs[
            (self._df_bs["financial_statement"] == financial_statement)
            & (self._df_bs["yearly"] == yearly)
        ]
    ...
    def income_statement(
        ...
        df_cis = self._df_cis[
            (self._df_cis["financial_statement"] == financial_statement)
            & (self._df_cis["yearly"] == yearly)
        ]
        df_cis.drop(columns=["financial_statement", "yearly"], inplace=True)
        df_is = self._df_is[
            (self._df_is["financial_statement"] == financial_statement)
            & (self._df_is["yearly"] == yearly)
        ]
    ...
    def cash_flow(
        ...
        df_cf = self._df_cf[
            (self._df_cf["financial_statement"] == financial_statement)
            & (self._df_cf["yearly"] == yearly)
        ]
    ...
    def equity_change(
        ...
        df_sce = self._df_sce[
            (self._df_sce["financial_statement"] == financial_statement)
            & (self._df_sce["yearly"] == yearly)
        ]
        df_sce.drop(columns=["financial_statement", "yearly"], inplace=True)
        return df_sce

    def quarterly_equity_change(
        self, corp_code: str, year: str, quarter: int, financial_statement: str = "CFS"
    ):
        """OpenDart 정기보고서 재무정보 - 자본변동표"""
    ...
    def _financial_statements(
        ...
        df_cis = self._transform_dart_financial_data(
            total_cis, financial_statement=financial_statement, yearly=yearly
        )
        df_is = self._transform_dart_financial_data(
            total_is, financial_statement=financial_statement, yearly=yearly
        )
        df_bs = self._transform_dart_financial_data(
            total_bs, financial_statement=financial_statement, yearly=yearly
        )
        df_cf = self._transform_dart_financial_data(
            total_cf, financial_statement=financial_statement, yearly=yearly
        )
        df_sce = self._transform_dart_financial_data(
            total_sce, financial_statement=financial_statement, yearly=yearly
        )
    ...
    def _transform_dart_financial_data(
        self, data: List[Dict[str, Any]], financial_statement: str, yearly: bool
    ) -> pd.DataFrame:
        """
        ...
        # 기간 라벨 생성
        df["period"] = df.apply(
            lambda x: get_period_label(x["reprt_code"], x["bsns_year"]), axis=1
        )
        ...
        for _, row in df.iterrows():
            ...
            if pd.notna(row["thstrm_add_amount"]) and row["reprt_code"] != "11011":
                result_data[account][f"{period}(+)"] = format_amount(
    ...
    def _parse_xml(self, xml_content, filename=None):
        ...
        for idx, table in enumerate(tables):
            ...
            if rows:
                ...
                result["tables"].append({"index": idx, "data": rows, "dataframe": df})
    ...
    def _to_dataframe(self, data: list[dict]) -> pd.DataFrame:
        ...
        # 정렬
        df = df.sort_values(
            ["bsns_year", "reprt_code", "ord"], ascending=[False, False, True]
        )
